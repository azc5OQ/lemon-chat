            // the UI object, split out of main.js (same script element, same closure - build.py
            // splices this file right before main.js). every method that renders or mutates the
            // page lives here: themes, member list sync, settings tabs, context menus, dialogs
            ...
            var UI = {
                ...
                // connect button on the login form: just starts the connection
                ...
                promote_offline_chat_context: function(client)
                {
                    if (client == null || typeof client.alias !== "string" || client.alias.length == 0) { return; }

                    let offline_context_id = "chat-context-offline-" + client.alias;
                    let offline_context = document.getElementById(offline_context_id);
                    if (offline_context == null) { return; }

                    let live_context_id = "chat-context-pm-" + client.client_id;
                    let live_context = document.getElementById(live_context_id);
                    let was_open = (g_current_chat_context_id == offline_context_id);

                    if (live_context == null)
                    {
                        // no live chat yet: the offline one simply becomes it, history and all
                        offline_context.id = live_context_id;

                        for (let i = 0; i < g_chat_context_array.length; i++)
                        {
                            if (g_chat_context_array[i].chat_context_id == offline_context_id) { g_chat_context_array[i].chat_context_id = live_context_id; }
                        }
                    }
                    else
                    {
                        ...
                        for (let i = g_chat_context_array.length - 1; i >= 0; i--)
                        {
                            if (g_chat_context_array[i].chat_context_id == offline_context_id) { g_chat_context_array.splice(i, 1); }
                        }
                    }
                    ...
                    // if we were reading/typing in it, keep it on screen and send live from now on
                    if (was_open == true)
                    {
                        ...
                        document.getElementById(live_context_id).style.display = "block";
                        UI.enable_inputs();
                    }
                ...
                identity_string_use_confirm_button: function()
                {
                    ...
                    if (identity_passphrase_string != null && identity_passphrase_string.length > 0)
                    {
                        ...
                        g_is_rsa_key_generated = false; // so the reconnect below waits for the NEW keypair
            ...
            var webrtc = {
                ...
                datachannel_onopen: function()
                {
                    ...
                    g_client_list[get_client_index_in_array_by_client_id(local_client_id)].audio_state = AUDIO_STATE.PUSH_TO_TALK_DISABLED_BUT_CAN_RECEIVE_AUDIO_FROM_OTHERS;
