            ...
            function websocket_worker_onmessage(e)
            {
                ...
                if (e.data.type == "create_websocket_object")
                {
                    ...
                    g_websocket_instance.onerror = function ()
                    {
                ...
                {
                    if (g_websocket_instance.readyState === 1)
                    {
            ...
            // autoconnect retry feedback: when the next attempt is due, and whether the last attempt
            // actually failed (so the login shows "connection failed, retrying in Xs" - not before)
            var g_next_reconnect_at = 0;
            ...
            var g_deep_idle_grace_until_timestamp = 0;
            ...
            // person's avatar one at a time so faces are there before anybody is clicked. previously an
            // avatar only arrived when you opened that person, so a fresh join showed a wall of blanks.
            // nothing new is asked of the server - these are the same per-client requests the click path
            // sends, just paced. set to false for fetch-on-demand, or raise the interval on slow links.
            // "simple" or "advanced" as reported by the android wrapper, "" in a browser. simple mode
            // means the simpledark look, and it must survive arriving either before or after the
            // startup theme pick - both orders happen depending on how fast the bridge answers
            var g_android_app_mode = "";

            // the android wrapper's simple/advanced mode drives the theme, both ways. simple locks the
            // messenger look; advanced restores the user's saved theme (or the device default). called at
            // startup and again whenever android re-pushes settings after a mode change - the old code
            // only handled "simple", so switching TO advanced changed nothing and looked stuck
            ...
            function apply_avatar_to_ui(client_id, base64)
            {
                ...
                if (parseInt(g_profile_avatar_client_id) === parseInt(client_id))
                {
                    ...
                    if (big != null)
                    {
                        if (has_avatar) { big.style.backgroundImage = "url(" + base64 + ")"; big.style.backgroundSize = "100% 100%"; }
                        else { big.style.backgroundImage = ""; }
                    }
                }
            }
            ...
            // push-to-talk key: pressing Q starts sending audio (ignored when the mic is always on)
            function document_onkeydown(event)
            {
                if (g_is_microphone_always_on == true)
                {
                    return;
                }
                if (event.which == 81)
            ...
            // releasing Q stops sending audio and clears the opus encoder's buffered frames
            ...
            function enter_deep_idle()
            {
                ...
                // activity transitions around call accept briefly fire onStop, dont re-idle mid-transition
                if (new Date().valueOf() < g_deep_idle_grace_until_timestamp)
                {
                    return;
                }
                ...
                // capture stays off until the next press
                set_microphone_capture_active(false);
            ...
            function exit_deep_idle()
            {
                ...
                g_deep_idle_grace_until_timestamp = new Date().valueOf() + 5000;
            ...
            function send_chat_message()
            {
                ...
                else if (g_chat_message_receiver_type == "user")
                {
                    if (get_client_by_client_id(chat_message_receiver_id) == null)
                    {
                        ...
                        if (live_client != null)
                        {
                            UI.promote_offline_chat_context(live_client);
            ...
            {
                return g_map_client_id_to_array_index.get(client_id);
            }
            ...
            function generate_html_for_single_client(client, is_local_client)
            {
                ...
                if (is_local_client)
                {
                    result = "<div class=\"connected-local-client connected-client " + is_channel_collapsed + has_alias_html_class + "\" data-connected-client-id=\"" + client_id + "\">\n\
                                ...
                                <div class='client-audio-state "+ client_audio_state_html_class + "' id=\"client-audio-state-" + client_id + "\"" + avatar_inline_style + ">\n\
                                </div>\n\
            ...
            function reset_chat_app_keep_identity()
            {
                ...
                g_map_client_id_to_array_index.clear(); // clear the mapp
            ...
            function process_audio_state_of_single_client(client)
            {
                ...
                g_client_list[get_client_index_in_array_by_client_id(client.client_id)].audio_state = client.audio_state;
            ...
            {
                // g_is_authenticated flips false in the websocket_worker_onclose handler, right before
                // the ui resets to the login screen - that is the moment the old connection is gone
                while (g_is_authenticated == true)
                {
                    await sleep(200);
                }

                while (g_is_rsa_key_generated == false)
                {
                    await sleep(200);
                }
            ...
            // waits until keys are ready, builds the ws/wss connection string, then has the
            // websocket worker open the socket sending the encrypted public key handshake onopen
            async function connect_to_server()
            {
                ...
                // a new attempt is in flight - clear the failed flag so the retry ticker does not
                // show "connection failed" over an attempt that has not resolved yet
                ...
                if (g_is_websocket_connected == false)
                {
                    ...
                    if (document.location.protocol == "https:")
                    {
                        ...
                        if (g_are_server_details_predefined == true && g_autoconnect_details.wss_port)
                        {
                            connection_port = g_autoconnect_details.wss_port;
                        }
                    }
                    ...
                    // say WHERE we are connecting, not just that we are. written here rather than at the
                    // top of connect_to_server because only now are the real host/port known: autoconnect
                    // details override the form fields, and an https page moves to the stunnel wss port
            ...
            function mainthread_onmessage(e)
            {
                ...
                {
                    // a lost/refused connection during autoconnect: mark it so the retry ticker can
                    // show "connection failed, retrying in Xs" instead of a bare "loading..."
            ...
            // autoconnect entry: connects immediately when predefined details allow it, and keeps
            // retrying every 30s while unauthenticated if reconnect is enabled
            function begin_connect_to_server()
            {
                if (g_are_server_details_predefined == true && g_is_autoconnect_without_user_action_active == true)
                {
                    connect_to_server();

                    if (g_is_reconnect_active)
                    {
                        // retry every 30s, but tick each second so the login page can show WHY it is
                        // waiting and HOW LONG - it used to sit on "connecting..." with no feedback
                        let RECONNECT_INTERVAL_MS = 30000;
                        g_next_reconnect_at = new Date().valueOf() + RECONNECT_INTERVAL_MS;
                        ...
                        window.setInterval(function()
                        {
                            ...
                            if (now >= g_next_reconnect_at)
                            {
                                connect_to_server();
                                g_next_reconnect_at = now + RECONNECT_INTERVAL_MS;
                                return;
                            }

                            // only claim "failed" once an attempt has actually errored/closed; until
                            // then the first attempt may still be in progress
                            if (g_last_connect_attempt_failed == true)
                            {
                                let seconds_left = Math.ceil((g_next_reconnect_at - now) / 1000);
                                let status_element = document.getElementById("another-buttons-loading-container-p");
                                if (status_element != null)
                                {
                                    status_element.innerHTML = "connection failed<br>retrying in " + seconds_left + "s";
                                }
                            }
                        }, 1000);
                    }
            ...
            async function window_onload()
            {
                ...
                g_data_processing_worker.postMessage({
                    ...
                    from_identity_string: use_persisted_identity,
                    identity_passphrase_string: use_persisted_identity ? persisted_identity : null
                });
