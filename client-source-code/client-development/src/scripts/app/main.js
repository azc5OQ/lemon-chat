...
function describe_client_for_profile(client)
{
    ...
    let shown_country = country_flag_code_for_client(client);
    if (shown_country != null && shown_country.length > 0) { parts.push(shown_country.toUpperCase()); }
...
async function websocket_connection_check(websocket)
{
    while (g_should_connection_check_be_running == true)
    {
        ...
        try
        {
            ...
            {
                let silence_ms = timestamp_now - g_connection_check_message_response_received_timestamp;
                ...
                if (try_start_fast_reconnect("no heartbeat reply for " + silence_ms + " ms") == true)
                {
                    break;
                }
...
function generate_html_for_single_client(client, is_local_client)
{
    ...
    let country_iso_code = country_flag_code_for_client(client);
...
// fast reconnect: the socket is gone but the page keeps everything (lists, chat, channel, keys) and
// the driver re-dials at once with the same identity, asking the server to adopt the session.
// returns false when a resume is not possible, so the caller runs the classic wipe instead
function try_start_fast_reconnect(reason)
{
    ...
    if (g_is_fast_reconnect_in_progress == true)
    {
        return false;
    }

    g_is_fast_reconnect_in_progress = true;
    g_is_fast_reconnect_resumed = false;
    g_fast_reconnect_pending_lists = null;
    ...
    custom_log("fast reconnect attempt (" + reason + ")");

    g_fast_reconnect_deadline_timer = setTimeout(function()
    {
        fast_reconnect_failed("no adopted session within " + (G_FAST_RECONNECT_DEADLINE_MS / 1000) + " s");
    }, G_FAST_RECONNECT_DEADLINE_MS);
...
// the one failure of a fast reconnect: the classic "connection lost" toast and the full wipe
function fast_reconnect_failed(reason)
{
    if (g_is_fast_reconnect_in_progress == false && g_is_fast_reconnect_resumed == false)
    {
        return;
    }

    g_is_fast_reconnect_in_progress = false;
    g_is_fast_reconnect_resumed = false;
    g_fast_reconnect_pending_lists = null;
    clearTimeout(g_fast_reconnect_deadline_timer);
    g_fast_reconnect_deadline_timer = null;

    console.log("%cfast reconnect failed: " + reason + " - back to the connect screen", "color: #ff4040; font-weight: bold; font-size: 14px;");
    custom_log("fast reconnect failed (" + reason + ")");
...
// the server adopted the session: everything on this page is still valid. the complete lists follow
// as on a login and are held back until the last one, see fast_reconnect_buffer_list
function fast_reconnect_succeeded()
{
    g_is_fast_reconnect_resumed = true;
    g_fast_reconnect_pending_lists = {};
    console.log("fast reconnect: the server adopted the session, waiting for the refreshed lists");
}

// resume: the four lists (channel, client, icon, tag) are applied together in one task, so the old
// ones are replaced in a single repaint and nothing on screen flickers. false = not resuming
function fast_reconnect_buffer_list(kind, value)
{
    ...
    if (g_fast_reconnect_pending_lists == null)
    {
        g_fast_reconnect_pending_lists = {};
    }

    g_fast_reconnect_pending_lists[kind] = value;

    if (g_fast_reconnect_pending_lists.channel != null && g_fast_reconnect_pending_lists.client != null
        && g_fast_reconnect_pending_lists.icon != null && g_fast_reconnect_pending_lists.tag != null)
    {
        fast_reconnect_apply_lists();
    }

    return true;
}

function fast_reconnect_apply_lists()
{
    let lists = g_fast_reconnect_pending_lists;
    ...
    custom_log("fast reconnect happened");
}
...
function mainthread_onmessage(e)
{
    ...
    else if (e.data.type == "websocket_worker_onclose")
    {
        ...
        if (g_is_fast_reconnect_in_progress == true)
        {
            fast_reconnect_failed("the resume socket closed");
        }
        else if (try_start_fast_reconnect("socket closed") == false)
        {
    ...
    else if (e.data.type == "websocket_worker_onerror")
    {
        ...
        if (g_is_fast_reconnect_in_progress == true)
        {
            fast_reconnect_failed("the resume socket failed");
        }
        else if (try_start_fast_reconnect("socket error") == false)
        {
            if (g_is_running_in_android_webview == false)
            {
    ...
    else if (e.data.type == "data_processing_worker__authentication_status")
    {
        if (e.data.value == "success")
        {
            ...
            if (g_is_fast_reconnect_in_progress == true && g_is_fast_reconnect_resumed == false)
            {
                fast_reconnect_failed("the server had no session to resume");
                return;
            }
            ...
            if (e.data.is_idle_mode_allowed && g_is_fast_reconnect_resumed == false)
            {
            ...
            if (g_is_client_running_under_touch_device && g_is_fast_reconnect_resumed == false)
            {
    ...
    {
        if (fast_reconnect_buffer_list("client", e.data.value) == false)
        {
    ...
    {
        if (fast_reconnect_buffer_list("channel", e.data.value) == false)
        {
    ...
    {
        if (fast_reconnect_buffer_list("tag", e.data.value) == false)
        {
    ...
    {
        if (fast_reconnect_buffer_list("icon", e.data.value) == false)
        {
    ...
    else if (e.data.type == "data_processing_worker__public_key_challenge_from_server")
    {
        ...
        let message_object = {
            message: {
                ...
                fast_reconnect: (g_is_fast_reconnect_in_progress == true)
            }
        };
