            ...
            var server_msg = {
                ...
                process_client_connect_from_server: function(msg)
                {
                    ...
                    UI.promote_offline_chat_context(single_client);
                ...
                // android-only: hands an incoming call (caller username + channel) to the
                // native layer via Android.JavaExportStartCall; alerts elsewhere
                ...
                process_channel_maintainer_id_from_server: function(msg)
                {
                    ...
                    {
                        console.log("local user is maintainer of channel '" + g_channel_list[get_channel_index_in_array_by_channel_id(g_channel_list, msg.message.channel_id)].name + "'");
                ...
                process_offline_chat_message_from_server: function(msg)
                {
                    ...
                    let html_to_append = "<div class=\"single-chat-message\">\n\
                                            <div class=\"single-message-content\">\n\
                                                <div class=\"single-chat-message-sender-username-container\">\n\
                                                    <p>" + sanitize_string(sender_alias) + "</p>\n\
                                                </div>\n\
                                                <div class=\"single-chat-message-sender-time\">\n\
                                                    <p>" + when_text + "</p>\n\
                                                </div>\n\
                    ...
                    {
                        let badge_row = document.querySelector('[data-connected-client-id="' + sender_client.client_id + '"] .connected-client-p-received-messages-number');
                        if (badge_row != null)
                        {
                            badge_row.style.display = "inline-block";
                            badge_row.innerHTML = "" + (parseInt(badge_row.innerHTML) + 1);
                        }
                    }
                ...
                process_client_list_from_server: function(msg)
                {
                    ...
                    for (let x = 0; x < g_chat_context_array.length; x++)
                    {
                        if (g_chat_context_array[x].type == "user")
                        {
                            ...
                            {
                                if (g_chat_context_array[x].chat_context_id == "chat-context-pm-" + msg.message.clients[y].id)
                                {
                                    break;
                                }
                                else
            ...
            var android_js_bridge = {
                ...
                {
                    g_is_microphone_always_on = true;
                },
                ...
                set_username_on_connect_android: function(username)
                {
                    ...
                    document.getElementById('connected-local-client-input').setAttribute('value', g_local_username);
                },

                set_audio_effects_status_android: function()
                {
                    g_are_sound_effects_enabled = false;

                    if (g_are_sound_effects_enabled == false)
                    {
                ...
                accept_current_settings_from_android: function(json_current_settings)
                {
                    ...
                    g_is_autoconnect_without_user_action_active = (settings_from_android.is_autoconnect_enabled == true);
                    g_is_microphone_always_on = (settings_from_android.is_microphone_always_on == true);
                    ...
                    // app-held identity: the android app stores the 200-char passphrase seed and passes it in,
                    // so the server sees the same client across launches, webview data clears and reinstalls.
                    // the page-load keygen already ran with a random seed - regenerate from the app's seed
                    // (same pattern as the identity import flow)
                    let android_identity_seed = null;

                    if (settings_from_android.identity_string != null && settings_from_android.identity_string.length >= 199)
                    {
                        android_identity_seed = settings_from_android.identity_string;

                        if (android_identity_seed != identity_string)
                        {
                            g_is_rsa_key_generated = false; // so the connect below waits for the NEW keypair

                            g_data_processing_worker.postMessage({
                                type: "mainthread__generate_rsa_keypair",
                                from_identity_string: true,
                                identity_passphrase_string: android_identity_seed
                            });
                        }
                    }

                    if (is_autoconnect_enabled && g_is_autoconnect_without_user_action_active)
                    {
                        (async function()
                        {
                            // keygen runs in the worker; connect only once OUR seed produced the live keypair
                            // (g_is_rsa_key_generated alone could flip true from the page-load random keygen).
                            // bounded so a dead worker cannot hang the connect forever
                            if (android_identity_seed != null)
                            {
                                let wait_rounds = 0;

                                while (identity_string != android_identity_seed && wait_rounds < 200)
                                {
                                    wait_rounds++;
                                    await sleep(50);
                                }
