// the client's whole rsa layer, replacing the vendored jsbn.js, rsa.js, rsa-sign.js,
// cryptico.js and biginteger.js. the modular exponentiation runs in rsa_keygen.wasm,
// everything else is native BigInt, aes-js and crypto.getRandomValues.
//
// every wire format below is byte-identical to what cryptico produced - the public key
// string, the pkcs#1 challenge and the "rsa(aeskey)?aes-cbc(payload)" envelope - so
// identities and messages stay compatible with clients that still run the old stack.

var g_rsa_keygen_wasm_exports = null;

// the module imports nothing and is ~23 KB. synchronous compilation is fine because
// every rsa caller lives in the data processing worker; the main thread only ever uses
// the diffie-hellman helper below, which needs no wasm
function get_rsa_keygen_wasm()
{
    if (g_rsa_keygen_wasm_exports == null)
    {
        ...
        g_rsa_keygen_wasm_exports = new WebAssembly.Instance(wasm_module, {}).exports;
    }
    return g_rsa_keygen_wasm_exports;
}
...
var lemon_crypto = (function ()
{
    ...
    // -----------------------------------------------------------------------
    // conversions, ported unchanged from cryptico: the envelope is built out of
    // these, so a single differing character would break older clients
    // -----------------------------------------------------------------------
    ...
    // zero padding to a whole number of aes blocks, and its inverse. not pkcs#7: this
    // is the padding cryptico shipped, and the envelope format is fixed by old clients
    ...
    // -----------------------------------------------------------------------
    // random material. the old stack drew this from jsbn's SecureRandom, an arcfour
    // pool seeded from Math.random at load time; getRandomValues is the csprng the
    // rest of the client already uses for key material
    // -----------------------------------------------------------------------
    ...
    // minimal big-endian bytes, the shape jsbn's toByteArray produced for a positive
    // value once its leading zero byte is skipped
    ...
    // base^exponent mod modulus on the wasm. the modulus must be odd, which every rsa
    // n, p and q is
    function wasm_modpow_hex(base_hex, exponent_hex, modulus_hex)
    {
        var wasm = get_rsa_keygen_wasm();
    ...
    // square and multiply over native BigInt. the diffie-hellman step runs on the main
    // thread, which never instantiates the wasm module, so it uses this instead
    ...
    // -----------------------------------------------------------------------
    // the key object. fields are native BigInt and carry the names the old RSAKey
    // used, so anything holding a key keeps working
    // -----------------------------------------------------------------------
    ...
    // pkcs#1 encryption of "text", as an even-length hex string
    ...
    // undoes the above; null when the padding does not check out
    ...
    // the identity keypair: the seeded prime walk in the wasm, which reproduces the
    // old jsbn walk bit for bit (see client-webassemblies/rsa-keygen)
    ...
    // the server's rsa challenge: pkcs#1 ciphertext, base64, no aes layer
