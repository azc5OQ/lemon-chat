            // android-webview.js is the android webview bridge: the implementations behind the
            // JavascriptJavaBridge__* page globals in android-bridge.js, which java calls into
            // this is android only; a desktop or website client wires none of these. it is included
            // in both bundles because it must live inside moduleFactory's scope, and node reaches
            // it through the export seam
            ...
            var android_js_bridge = {
                ...
                send_come_from_idle_mode_request_android: function(channelId = null)
                {
                    ...
                    if (channelId != null)
                    {
                        mark_call_accept_presence_grace();
                    }
                ...
                // java calls this on ACTION_ACCEPT_CALL, so the webview's own idle logic knows a
                // call was just accepted; node learns it through its come-from-idle instead
                ...
                // node's connection phase relayed by java. it feeds the page's own status
                // machinery, which the ticker keeps repainting, and keeps the spinner up while node works
                ...
                // java says node is connected or the app came to front. the dial decision belongs
                // to request_connect; this only self-heals a lost settings push first
                ...
                accept_current_settings_from_android: function(json_current_settings)
                {
                    ...
                    // the theme is applied first, before anything below can throw, because the
                    // settings json is built up incrementally on the java side and fields like
                    // metadata_keys may simply not exist yet. one missing field used to kill this
                    // whole function halfway, and the theme code at its old spot near the end never ran
                    ...
                    // these fields may be absent from an incremental first-run json, and undefined
                    // must count as false (undefined == false is FALSE in js, so the == false checks
                    // below would silently skip both branches otherwise). an absent field means ON,
                    // because a fresh install's json lacks it, and reading that as off left node
                    // never connecting, with no way to trigger it from the ui
                    ...
                    // because routing state through a click handler meant it could not be applied without a ui,
                    // and the handler's `= !flag` would have inverted it the moment a second writer existed. the
                    // mic and the repaint stay behind UI.*, so a runtime with no ui sets the flag and touches no audio
                    ...
                    // on android the connection details come from the java settings window, so the
                    // in-page ip/port form is pointless and is hidden fully (display:none, not just
                    // invisible, so it leaves no dead gap). the connect-button visibility block in
                    // window_onload is web-only, so it is driven here: autoconnect off means the
                    // user needs a connect button
                    ...
                    // no unconditional spinner hold here, because request_connect raises the spinner
                    ...
                    // own the connection. whether to actually dial is decided inside request_connect,
