...
fn check_if_username_change_allowed(client_id: u64,  clients: &mut HashMap<u64, Client>, client_stored_data: &mut Vec<ClientStoredData>, message: &serde_json::Value) -> bool {
    ...
    let new_desired_username: String = message["message"]["new_username"].as_str().unwrap().to_string();
    ...
    for (_key, client) in clients {
        if client.username == new_desired_username {
            println!("username taken, entry in clients");
            return false;
        }
    }

    for data in client_stored_data {
        if data.username == new_desired_username {
            println!("username taken, entry in client_stored_data");
            return false;
        }
    }
