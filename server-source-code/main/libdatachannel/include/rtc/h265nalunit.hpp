...
struct RTC_CPP_EXPORT H265NalUnitFragmentHeader {
	 ...
	 */
			...
			bool isEnd() const
			{
				return (_first >> 6) & 0x01;
			}
			uint8_t unitType() const
			{
				return _first & 0b11'1111; }
	...
	void setUnitType(uint8_t type) {
					_first = (_first & 0b1100 '0000) | (type & 0b11' 1111); }
};
#pragma pack(pop)
	...
	{
		static std::vector<binary> GenerateFragments(const std::vector<H265NalUnit> &nalus, size_t maxFragmentSize);
		...
		H265NalUnit(binary &&data)
			: NalUnit(std::move(data))
		{
		}
		H265NalUnit()
			: NalUnit(NalUnit::Type::H265)
		{
		}
		...
		bool forbiddenBit() const
		{
			return header()->forbiddenBit();
		}
		uint8_t unitType() const
		{
			return header()->unitType();
		}
		uint8_t nuhLayerId() const
		{
			return header()->nuhLayerId();
		}
		uint8_t nuhTempIdPlus1() const
		{
			return header()->nuhTempIdPlus1();
		}
		...
		void setForbiddenBit(bool isSet)
		{
			header()->setForbiddenBit(isSet);
		}
		void setUnitType(uint8_t type)
		{
			header()->setUnitType(type);
		}
		void setNuhLayerId(uint8_t nuhLayerId)
		{
			header()->setNuhLayerId(nuhLayerId);
		}
		void setNuhTempIdPlus1(uint8_t nuhTempIdPlus1)
		{
			header()->setNuhTempIdPlus1(nuhTempIdPlus1);
		}
	...
	{
		[[deprecated]] static std::vector<shared_ptr<H265NalUnitFragment> > fragmentsFrom(shared_ptr<H265NalUnit> nalu, uint16_t maxFragmentSize);
		...
		H265NalUnitFragment(FragmentType type, bool forbiddenBit, uint8_t nuhLayerId, uint8_t nuhTempIdPlus1, uint8_t unitType, binary data);

		uint8_t unitType() const
		{
			return fragmentHeader()->unitType();
		}
		...
		void setUnitType(uint8_t type)
		{
			fragmentHeader()->setUnitType(type);
		}
		...
		H265NalUnitHeader *fragmentIndicator()
		{
			return reinterpret_cast<H265NalUnitHeader *>(data());
		}
	...
	{
	    public:
		static const uint16_t defaultMaximumFragmentSize = uint16_t(RTC_DEFAULT_MTU - 12 - 8 - 40); // SRTP/UDP/IPv6
