...
struct RTC_CPP_EXPORT NalUnitHeader
{
	...
	{
		_first = (_first & 0x9F) | ((nri & 0x03) << 5);
	}
	void setUnitType(uint8_t type)
	{
		_first = (_first & 0xE0) | (type & 0x1F);
	}
};
...
struct RTC_CPP_EXPORT NalUnitFragmentHeader
{
	...
	bool reservedBit6() const
	{
		return (_first >> 5) & 0x01;
	}
	bool isEnd() const
	{
	...
	void setStart(bool isSet)
	{
		_first = (_first & 0x7F) | (isSet << 7);
	}
	void setEnd(bool isSet)
	{
		_first = (_first & 0xBF) | (isSet << 6);
	}
	void setReservedBit6(bool isSet)
	{
		_first = (_first & 0xDF) | (isSet << 5);
	}
	void setUnitType(uint8_t type)
	{
		_first = (_first & 0xE0) | (type & 0x1F);
...
{
	static std::vector<binary> GenerateFragments(const std::vector<NalUnit> &nalus, size_t maxFragmentSize);
...
struct RTC_CPP_EXPORT NalUnitFragmentA : NalUnit
{
	...
	[[deprecated]] static std::vector<shared_ptr<NalUnitFragmentA> > fragmentsFrom(shared_ptr<NalUnit> nalu, uint16_t maxFragmentSize);
	...
	NalUnitFragmentA(FragmentType type, bool forbiddenBit, uint8_t nri, uint8_t unitType, binary data);
