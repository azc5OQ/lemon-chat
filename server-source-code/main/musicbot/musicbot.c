#include "../../libopus-1.5.2/include/opus.h"
#include "../log/log.h"
...
{
	if (music_bot->music_bot_client_extension.songs[song_id].is_existing == TRUE)
	{
...
void musicbot__add_song(char *song_name, ubyte *mp3_data_buffer, uint64 mp3_data_buffer_length, client_t *music_bot)
{
	...
	if (music_bot->music_bot_client_extension.music_bot_songs_count == MUSIC_BOT_MAX_FILE_COUNT)
	{
	...
	{
		song_in_loop = &music_bot->music_bot_client_extension.songs[i];
		if (song_in_loop->is_existing == FALSE)
		{
			song_in_loop->mp3_data_buffer = mp3_data_buffer;
			song_in_loop->mp3_data_buffer_length = mp3_data_buffer_length;

			DBG_MUSIC_BOT log_info("%s %s %s", "musicbot__add_song added song -> ", song_name, "\n");

			clib__copy_memory((void *)song_name, song_in_loop->song_name, clib__utf8_string_length(song_name), SONG_NAME_MAX_LENGTH);
