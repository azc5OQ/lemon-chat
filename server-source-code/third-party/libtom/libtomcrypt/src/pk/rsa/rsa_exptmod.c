                      ...
                      rsa_key *key)
{
   ...
   printf("%s", "rsa_exptmod 2 \n");

   if ((err = mp_read_unsigned_bin(tmp, (unsigned char *)in, (int)inlen)) != CRYPT_OK)
        {
          printf("%s", "rsa_exptmod 234 \n");
          goto error; 
        }

   printf("%s", "rsa_exptmod 22 \n");

   /* sanity check on the input */
   if (mp_cmp(key->N, tmp) == LTC_MP_LT) {
      err = CRYPT_PK_INVALID_SIZE;
      printf("%s", "rsa_exptmod 23 \n");

      goto error;
   }

   printf("%s", "rsa_exptmod 3 \n");
   ...
   if (which == PK_PRIVATE) {
      ...
      printf("%s", "rsa_exptmod 4 \n");

      /* exptmod it */
      if ((err = mp_exptmod(tmp, key->e, key->N, tmp)) != CRYPT_OK)                                { goto error; }
   }

   printf("%s", "rsa_exptmod 5 \n");
