...
static int rsa_check_context(mbedtls_rsa_context const *ctx, int is_priv, int blinding_needed)
{
        ...
        //printf("%s","rsa_check_context STEP 1  \n");
    ...
    if (ctx->len != mbedtls_mpi_size(&ctx->N) ||
        ...
        //printf("%s","rsa_check_context error 1  \n");
        return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
    }


        //printf("%s","rsa_check_context STEP 2 \n");
    ...
    if (mbedtls_mpi_cmp_int(&ctx->N, 0) <= 0 ||
        ...
        //printf("%s","rsa_check_context error 2  \n");
    ...
    if (is_priv &&
        (mbedtls_mpi_cmp_int(&ctx->P, 0) <= 0 ||
         ...
         mbedtls_mpi_get_bit(&ctx->Q, 0) == 0)) {
                    //printf("%s","rsa_check_context error 3  \n");
    ...
    if (mbedtls_mpi_cmp_int(&ctx->E, 0) <= 0) {
                            //printf("%s","rsa_check_context error 4  \n");
        return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
    }
    ...
    if (is_priv && mbedtls_mpi_cmp_int(&ctx->D, 0) <= 0) {
                            //printf("%s","rsa_check_context error 5  \n");
        return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
    }
#else
    if (is_priv &&
        (mbedtls_mpi_cmp_int(&ctx->DP, 0) <= 0 ||
         mbedtls_mpi_cmp_int(&ctx->DQ, 0) <= 0)) {
                                //printf("%s","rsa_check_context error 6  \n");
        return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
    }
#endif /* MBEDTLS_RSA_NO_CRT */
    ...
    if (is_priv && blinding_needed &&
        (mbedtls_mpi_cmp_int(&ctx->P, 0) <= 0 ||
         mbedtls_mpi_cmp_int(&ctx->Q, 0) <= 0)) {
                                            //printf("%s","rsa_check_context error 7  \n");
    ...
    if (is_priv &&
        mbedtls_mpi_cmp_int(&ctx->QP, 0) <= 0) {
                                                        //printf("%s","rsa_check_context error 8  \n");

        return MBEDTLS_ERR_RSA_BAD_INPUT_DATA;
    }
#endif
                                                        //printf("%s","rsa_check_context no error  \n");
                       ...
                       unsigned char *output)
{
    ...
    if (rsa_check_context(ctx, 0 /* public */, 0 /* no blinding */)) {
        //printf("%s","mbedtls_rsa_public MBEDTLS_ERR_RSA_BAD_INPUT_DATA eh \n");
    ...
    if (mbedtls_mpi_cmp_mpi(&T, &ctx->N) >= 0) {
        ...
        //printf("%s","mbedtls_rsa_public MBEDTLS_ERR_MPI_BAD_INPUT_DATA \n");
...
cleanup:
    ...
    if (mbedtls_mutex_unlock(&ctx->mutex) != 0) {
         //printf("%s","mbedtls_rsa_public MBEDTLS_ERR_THREADING_MUTEX_ERROR \n");
    ...
    if (ret != 0) {
        //printf("%s","mbedtls_rsa_public MBEDTLS_ERR_RSA_PUBLIC_FAILED \n");
        return MBEDTLS_ERROR_ADD(MBEDTLS_ERR_RSA_PUBLIC_FAILED, ret);
    }


    //printf("%s","success no error \n");
