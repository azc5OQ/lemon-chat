...
extern "C" {
	 ...
	 */
	#define MESSAGE_LENGTH (2048 * 16)
	/**
	 * @brief Maximum frame/message length.
	 */
	#define MAX_FRAME_LENGTH (16*1024*1024*4)
	/**
	...
	struct ws_events
	{
		...
		void (*onmessage)(ws_cli_conn_t *client,
			unsigned char *msg, uint64_t msg_size, int type);
	};
    ...
    extern char* get_forwarded_ip_address(char *hsrequest);
