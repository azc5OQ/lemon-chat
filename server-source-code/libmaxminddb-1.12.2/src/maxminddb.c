...
dump_entry_data_list(StringBuffer *stream,
                     MMDB_entry_data_list_s *entry_data_list,
                     int indent,
                     int *status);
static void print_indentation(StringBuffer *sb, int i);
...
char* MMDB_dump_entry_data_list(StringBuffer* stream, MMDB_entry_data_list_s *const entry_data_list, int indent)
{
    int status;
...
/*
static MMDB_entry_data_list_s *dump_entry_data_list(StringBuffer *stream, MMDB_entry_data_list_s *entry_data_list, int indent, int *status)
{
    switch (entry_data_list->entry_data.type)
    {
        case MMDB_DATA_TYPE_MAP:
        {
            ...
            for (entry_data_list = entry_data_list->next; size && entry_data_list; size--)
            {
                ...
                char *key = mmdb_strndup(entry_data_list->entry_data.utf8_string, entry_data_list->entry_data.data_size);
                if (NULL == key)
                {
                    *status = MMDB_OUT_OF_MEMORY_ERROR;
                    return NULL;
        ...
        {
            char *string = mmdb_strndup(entry_data_list->entry_data.utf8_string, entry_data_list->entry_data.data_size);
            if (NULL == string)
            {
                *status = MMDB_OUT_OF_MEMORY_ERROR;
                return NULL;
        ...
        {
            char *hex_string = bytes_to_hex(entry_data_list->entry_data.bytes, entry_data_list->entry_data.data_size);
            if (NULL == hex_string)
            {
                *status = MMDB_OUT_OF_MEMORY_ERROR;
                return NULL;
        ...
        case MMDB_DATA_TYPE_UINT128:
        {
...
#else
            {
                ...
                fprintf(stream, "\"0x%016" PRIX64 "%016" PRIX64 "\"", high, low);
            }
#endif
            entry_data_list = entry_data_list->next;
        }
...
static MMDB_entry_data_list_s *dump_entry_data_list(StringBuffer *stream, MMDB_entry_data_list_s *entry_data_list, int indent, int *status) {
    switch (entry_data_list->entry_data.type) {
        case MMDB_DATA_TYPE_MAP: {
            ...
            print_indentation(stream, indent);
            append_to_string_buffer(stream, "{\n");
            indent += 2;
            ...
            for (entry_data_list = entry_data_list->next; size && entry_data_list; size--) {
                ...
                print_indentation(stream, indent);
                append_to_string_buffer(stream, "\"");
                append_to_string_buffer(stream, key);
                append_to_string_buffer(stream, "\": ");
                free(key);
        ...
        case MMDB_DATA_TYPE_UTF8_STRING: {
            ...
            append_to_string_buffer(stream, "\"");
            append_to_string_buffer(stream, string);
            append_to_string_buffer(stream, "\"");
        ...
        case MMDB_DATA_TYPE_BYTES: {
            ...
            print_indentation(stream, indent);
            append_to_string_buffer(stream, "\"");
            append_to_string_buffer(stream, hex_string);
            append_to_string_buffer(stream, "\"");
            free(hex_string);
            entry_data_list = entry_data_list->next;
        }
        ...
        case MMDB_DATA_TYPE_DOUBLE: {
            char buffer[64];
            sprintf(buffer, "%f", entry_data_list->entry_data.double_value);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
        break;
        ...
        case MMDB_DATA_TYPE_FLOAT: {
            char buffer[64];
            sprintf(buffer, "%f", entry_data_list->entry_data.float_value);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
        ...
        case MMDB_DATA_TYPE_UINT16: {
            char buffer[32];
            sprintf(buffer, "%u", entry_data_list->entry_data.uint16);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
        ...
        case MMDB_DATA_TYPE_UINT32: {
            char buffer[32];
            sprintf(buffer, "%u", entry_data_list->entry_data.uint32);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
        ...
        case MMDB_DATA_TYPE_BOOLEAN: {
            print_indentation(stream, indent);
            append_to_string_buffer(stream, entry_data_list->entry_data.boolean ? "true" : "false");
            entry_data_list = entry_data_list->next;
        }
        break;
        ...
        case MMDB_DATA_TYPE_UINT64: {
            char buffer[32];
            sprintf(buffer, "%" PRIu64, entry_data_list->entry_data.uint64);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
        ...
        case MMDB_DATA_TYPE_UINT128: {
...
#if MMDB_UINT128_IS_BYTE_ARRAY
            {
                ...
                append_to_string_buffer(stream, "\"0x");
                append_to_string_buffer(stream, hex_string);
                append_to_string_buffer(stream, "\"");
                free(hex_string);
            }
#else
            {
                ...
                uint64_t low = (uint64_t)entry_data_list->entry_data.uint128;
                char buffer[64];
                sprintf(buffer, "\"0x%016" PRIX64 "%016" PRIX64 "\"", high, low);
                append_to_string_buffer(stream, buffer);
            }
#endif
            entry_data_list = entry_data_list->next;
        ...
        case MMDB_DATA_TYPE_INT32: {
            char buffer[32];
            sprintf(buffer, "%d", entry_data_list->entry_data.int32);
            print_indentation(stream, indent);
            append_to_string_buffer(stream, buffer);
            entry_data_list = entry_data_list->next;
        }
...
/*
static MMDB_entry_data_list_s *
dump_entry_data_list_backup(FILE *stream,
                     MMDB_entry_data_list_s *entry_data_list,
                     int indent,
                     int *status) {
    switch (entry_data_list->entry_data.type) {
...
static void print_indentation(StringBuffer *sb, int i) {
    char buffer[1024];
    int size = i >= 1024 ? 1023 : i;  // Ensure we don't overflow the buffer
    memset(buffer, 32, (size_t)size);  // Fill the buffer with spaces
    buffer[size] = '\0';  // Null-terminate the string

    append_to_string_buffer(sb, buffer);  // Append the indentation to the StringBuffer
}
